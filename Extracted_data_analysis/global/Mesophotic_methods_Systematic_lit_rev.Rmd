...
```{r}
quad_design <- subset(raw,grepl("quad", samplingUnit) | grepl("quad", sampling) )
```

```{r}
quad_design <- subset(quad_design,grepl("trans", design) |  grepl("trans", meso_design)   )
```
...
molecular_notaxa <- raw %>%
  filter(taxonomic_publication != "taxa") 
