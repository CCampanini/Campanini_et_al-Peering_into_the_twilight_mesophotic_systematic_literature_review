...
title: "Peering into the twilight - a systematic review on visual-based investigation
  of worldwide mesophotic ecosystems"
...
```{r}
raw <- read_excel(here("Extracted_data_analysis","Input_data","extracted_raw.xlsx"))
```
...
writexl::write_xlsx(ecoreg_tot,
           here("Extracted_data_analysis","global","Output-Transformed_data","eco_reg_tot.xlsx"))
```
...
writexl::write_xlsx(realms, 
           here("Extracted_data_analysis","global","Output-Transformed_data","Realm_count.xlsx"))
```
...
```{r Calculate the number of studies per latitude zone}
env <- raw[,c (
...
writexl::write_xlsx(list(env_tot,realms, prov_tot, prov_per_record_df, ecoreg_tot, ecoreg_per_record_df),
           here("Extracted_data_analysis","global","Output-Transformed_data","geographical_distribution_summary.xlsx"))
```

```{r clean latitudinalRegion, echo=FALSE, results='hide'}
rm(env, env_lvls, ecoreg_lvls, meow, ecoreg, prov, location, ecoreg_per_record_df, prov_tot, prov_per_record_df)
```
...
```{r export treemap_counts, results= "hide"}
...
write.xlsx(count_results_method,
           file = here("Extracted_data_analysis",                        "global",                        "Output-Transformed_data",
...
lapply(names(treemap_counts), function(x){
  ggsave(filename=paste0("treemap",x,".tiff"), 
         plot=treemap_counts[[x]],
         units = "in", width=12, height=4, dpi=300, compression = 'lzw',
         path = here("Extracted_data_analysis","global",                        "Output-Visualization","Treemaps_methods"),
         create.dir = T
         )
  })
...
```{r}
split_tot[[6]] %<>%
  filter(indicators != "taxo") %>%
  mutate(indicators = case_when(indicators == "Abundance" ~ "Abundance (346)",
                               indicators == "Cover" ~ "Cover (312)",
                               indicators == "Diversity" ~ "Diversity  (458)",
                               indicators == "Habitat" ~ "Habitat (181)",
                               indicators == "Morphological" ~ "Morphology (244)",
                               indicators == "Size" ~ "Size (218)",
                               indicators == "Biomass" ~ "Biomass  (68)",
                               indicators == "Density" ~ "Density (219)",
                               indicators == "growth" ~ "Growth rate (42)",
                               indicators == "Health" ~ "Health (90)",
                               indicators == "Physiological" ~ "Physiological indicator (127)",
                               indicators == "ALDFGs" ~ "ALDFGs (64)",
                               indicators == "Litter" ~ "Litter (55)",
                               indicators == "Behaviour" ~ "Behaviour (40)",
                               indicators == "Trophic" ~ "Trophic (65)",
                               indicators == "impact" ~ "Impact (16)",
                               indicators == "surv" ~ "Survival (29)",
                               indicators == "geneticSequences" ~ "Genetic sequence (160)",
                               indicators == "rugosity" ~ "Rugosity (1)",
                               indicators == "func" ~ "Functional indicators (20)",
                               indicators == "sex" ~ "Sex (22)",
                               indicators == "Mortality" ~ "Mortality (27)",
                               indicators == "Recruitment" ~ "Recruitment (25)",
                               indicators == "NIS" ~ "Non-indigenous species (24)",
                               indicators == "complexity" ~ "Complexity (6)",
                               indicators == "disease" ~ "Diseases (7)",
                               indicators == "economicValue" ~ "Economic value (1)",
                               indicators == "ecoser" ~ "Ecosystem services (3)",
                               indicators == "complexity" ~ "Complexity (6)",
...
lapply(names(barchart_counts), function(x){
  ggsave(filename=paste0("bar_",x,".tiff"), 
         ...
         units = "in", width=12, height=4, dpi=300, compression = 'lzw',
         path = here("Extracted_data_analysis",                        "global",                        "Output-Visualization","Barcharts_split"),
         create.dir = T
         )
  })
...
lapply(names(barchart_split_methods), function(x){
  ggsave(filename=paste0("bar_",x,".tiff"), 
         ...
         units = "in", width=12, height=4, dpi=300, compression = 'lzw',
         path = here("Extracted_data_analysis",                        "global",                        "Output-Visualization","Barcharts_split_methods"), create.dir = T
         )
  })
...
```{r}
raw$mininumSurveyedDepthInMeters <- as.numeric(raw$mininumSurveyedDepthInMeters)
...
for (i in 1:nrow(pubs_depth)) {
  ...
  pubs_depth$count[i] <- sum(raw$maximumSurveyedDepthInMeters >= bin_start & raw$mininumSurveyedDepthInMeters < bin_end, na.rm = TRUE)
}
...
```{r Export pubs per depth bar plot, echo = F, results='hide'} 
ggsave("pubs_depth.png", 
       units = "in", width = 8, height = 6,
       path = here ("Extracted_data_analysis",                        "global",                        "Output-Visualization"))
```

## Publication per depth by latitudinalRegion

```{r Publication per depth by latitudinalRegion}
...
for (i in seq_along(bins)) {
  ...
  for (lat in unique(raw$latitudinalRegion)) {
    ...
    count <- sum(subset_data$maximumSurveyedDepthInMeters >= bin_start &
                 subset_data$mininumSurveyedDepthInMeters < bin_end, na.rm = TRUE)
...
# Plot
pub_depth_lat_bar <- ggplot(pub_depth_lat, aes(x = depth_bin, y = count, fill = latitudinalRegion)) +
  geom_bar(stat = "identity", position = "stack", alpha = 0.8) +
  labs(x = "Depth interval (m)", y = "Number of publications") +
  theme_minimal() +
  coord_flip() +
  scale_y_continuous(position = "right") +
  scale_fill_manual(values = c("#2a788e", "black", "#440154", "darkolivegreen3", "darkorange")) +
  theme(legend.position = "bottom")

pub_depth_lat_bar
```

Export bar chart as pub_depth_env.png 

```{r, echo = F, results='hide'}
ggsave("pub_depth_lat_bar.png", 
       plot = pub_depth_lat_bar,
       units = "in", width = 8, height = 6,
       path = here ("Extracted_data_analysis",                        "global",                        "Output-Visualization"))
```
...
platform_depth_lat <- raw %>%
  select(File, mininumSurveyedDepthInMeters,maximumSurveyedDepthInMeters,
...
platform_lat_lvls<- platform_lat_lvls[! platform_lat_lvls %in% c("",NA,"notapp",",","nd")]

platform_depth_lat[,platform_lat_lvls] <- sapply(platform_lat_lvls, 
                                      function(y){
                                        str_count(platform_depth_lat$meso_samplingPlatform, y)
                                        }
                                      )

rm(platform_lat_lvls)

platform_depth_lat[,-c(1:17)] <- lapply(platform_depth_lat[,-c(1:17)], as.numeric)

platform_depth_lat[,c(1:17)] <- lapply(platform_depth_lat[,c(1:17)], as.character)
...
```{r  platform_depth_lat_plot barplot}
...
platform_depth_lat_plot <- ggplot(expanded_data, aes(x = bin_label, fill = latitudinalRegion)) +
  ...
  scale_fill_manual(values = c("#2a788e", "#440154", "darkolivegreen3", "darkorange"))+
  theme(legend.position = "none")
...
```{r Export bar plot of most common platform per depth interval by latitudinalRegion, results= "hide"}
...
```{r Export bar plot of technique by decade, results= "hide"}
...
```{r}
dir_ind_by_decade_lat <- dir_ind_by_decade_lat [, c("meso_direct_indirect","decade","latitudinalRegion")] %>%
  group_by(decade,meso_direct_indirect, latitudinalRegion) %>% 
  tally()

colnames(dir_ind_by_decade_lat) <- c("Decade", "Technique","latitudinalRegion","Count")
...
```{r Make bar plot of technique by decade and latitudinalRegion}
technique_decade__env_barplot <- ggplot(subset(dir_ind_by_decade_lat,
              dir_ind_by_decade_lat$Decade != "1970-1979" & dir_ind_by_decade_lat$latitudinalRegion != "polar+temperate"),
       aes(x=Technique, y=Count, 
...
```{r Export bar plot of technique by decade and latitudinalRegion, results= "hide"}
...
```{r export field_ind_heat, results= "hide"}
...
```{r Export sampling_simplified_treemap, results= "hide"}
...
```{r Export data_type_treemap, results= "hide"}
...
```{r Export habit_donut, results= "hide"}
...
```{r Export the substrate donut plot, results= "hide"}
...
```{r Export the benthic_position donut plot, results= "hide"}
...
ggplot(target_level,
  ...
  labs(title = "Target level")+
    theme(plot.title = element_text(hjust = 0.5, size= 20),
          legend.position = "none")+
  scale_fill_manual(values= c("#66C2A5","#E78AC3" , "#8DA0CB","#FFD92F","#B3B3B3", "#E5C494","#FC8D62","#A6D854"))
...
```{r export group_bar.png, results= "hide"}
...
new_spec[,new_spec_lvls] <- sapply(new_spec_lvls, 
                                      function(y){
                                        str_count(new_spec$detected_meso_other_taxa, y)
                                        }
                                      )
...
new_spec_plot <-   ggplot(new_spec_tot, 
  ...
  labs(y = "N of Publications with new species descriptions", x = "Taxonomic group") +
  theme_minimal() +
  coord_flip()+  # optional: flips for horizontal bars
  theme(legend.position ="none")+
...
```{r export new_species_bar.png, results= "hide"}
...
```{r}
nrow(subset(platform_depth_lat,platform_depth_lat$ccr_direct == 1 & grepl("tropical",platform_depth_lat$latitudinalRegion)))
```

```{r}
nrow(subset(platform_depth_lat,platform_depth_lat$ccr_direct == 1 & grepl("temperate",platform_depth_lat$latitudinalRegion)))
```

```{r}
nrow(subset(platform_depth_lat,platform_depth_lat$hov == 1 & grepl("tropical",platform_depth_lat$latitudinalRegion)))
```

```{r}
nrow(subset(platform_depth_lat,platform_depth_lat$hov == 1 & grepl("temperate",platform_depth_lat$latitudinalRegion)))
```
...
colnames(raw)
print(paste("Publications on Corallium rubrum :", nrow(subset(raw, grepl("rubrum", targetTaxa)))))

print(paste("Publications on Paramuricea clavata:", nrow(subset(raw, grepl("clavata", targetTaxa)))))

print(paste("Publications on Montastraea cavernosa:", nrow(subset(raw, grepl("cavernosa", targetTaxa)))))
```
