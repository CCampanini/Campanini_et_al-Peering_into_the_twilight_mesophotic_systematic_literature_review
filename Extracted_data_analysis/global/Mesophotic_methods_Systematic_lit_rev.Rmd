---
title: "SYSTEMATIC REVIEW ON INVESTIGATION METHODS OF MESOPHOTIC latitudinalRegionS WITH A FOCUS ON BENTHIC COMMUNITIES"
...
# SYSTEMATIC REVIEW ON INVESTIGATION METHODS OF MESOPHOTIC latitudinalRegionS WITH A FOCUS ON BENTHIC COMMUNITIES
...
lapply(names(barchart_counts), function(x){
  ggsave(filename=paste0("bar_",x,".png"), 
         plot=barchart_counts[[x]],
         units = "in", width=12, height=4, dpi=300,
         path = here("Extracted_data_analysis",                        "global",                        "Output-Visualization","Barcharts_split"), create.dir = T
         )
  })
```
...
technique_decade__env_barplot <- ggplot(subset(dir_ind_by_decade_lat,
  ...
  theme_classic()+
  scale_fill_manual(values= c("#2a788e","#440154","darkolivegreen3","darkorange"),
                    labels = c("polar (28)", "temperate (523)", "temperate+tropical (15)", "tropical (425)")) +
  theme(legend.position = "bottom",
        legend.text = element_text(size = 10),
        axis.text.y = element_text(size=8.7),
        axis.title.x = element_blank(),
...
```{r Export bar plot of technique by decade and latitudinalRegion}
...
field_merged %<>%
  rename(`Biodiversity and \n community structure (412)` = field_commdesc,
         `Distribution and \n connectivity (324)` = field_dist,
         `Conservation and \n management (121)` = field_cons,
         `Disturbances (254)` = field_impass,
         `Mapping (180)` = field_map,
         `Physiology (162)` = field_physio,
         `Taxonomy (148)` = field_taxo,
         `Molecular ecology (159)` = field_Molecular_ecology,
         `Acoustics (127)` = field_Acoustics,
         `Environmental drivers and \n spatio-temporal patterns (267)` = field_environmental_drivers
         )
colSums(field_merged)
```
...
detected %<>%
  ...
  filter(across(everything(), ~ !grepl("notapp", .x)))
