...
title: "Peering into the twilight - a systematic review on visual-based investigation
  of mesophotic ecosystems - TROPICAL"
...
## Subset for studies encompassing tropical latitudinal region
...
write.xlsx(count_results_method,
           file = here("Extracted_data_analysis",                        "tropical",                        "Output-Transformed_data",
...
lapply(names(treemap_counts), function(x){
  ggsave(filename=paste0("treemap",x,".tiff"), 
         plot=treemap_counts[[x]],
         units = "in", width=12, height=4, dpi=300, compression = 'lzw',
         path = here("Extracted_data_analysis","tropical",                        "Output-Visualization","Treemaps_methods"),
         create.dir = T
         )
  })
...
lapply(names(barchart_counts), function(x){
  ggsave(filename=paste0("bar_",x,".tiff"), 
         ...
         units = "in", width=12, height=4, dpi=300, compression = 'lzw',
         path = here("Extracted_data_analysis",                        "tropical",                        "Output-Visualization","Barcharts_split"),
         create.dir = T
         )
  })
...
lapply(names(barchart_counts), function(x){
  ggsave(filename=paste0("bar_",x,".tiff"), 
         plot=barchart_counts[[x]],
         units = "in", width=12, height=4, dpi=300, compression = 'lzw',
         path = here("Extracted_data_analysis",                        "tropical",                        "Output-Visualization","Barcharts_split_methods"), create.dir = T
         )
  })
...
lapply(names(barchart_counts), function(x){
  ggsave(filename=paste0("bar_",x,".png"), 
         plot=barchart_counts[[x]],
         units = "in", width=12, height=4, dpi=300,
         path = here("Extracted_data_analysis",                        "tropical",                        "Output-Visualization","Barcharts_split"), create.dir = T
         )
  })
```
...
```{r}
tropical$mininumSurveyedDepthInMeters <- as.numeric(tropical$mininumSurveyedDepthInMeters)
tropical$maximumSurveyedDepthInMeters <- as.numeric(tropical$maximumSurveyedDepthInMeters)
...
for (i in 1:nrow(pubs_depth)) {
  ...
  pubs_depth$count[i] <- sum(tropical$maximumSurveyedDepthInMeters >= bin_start & tropical$mininumSurveyedDepthInMeters < bin_end, na.rm = TRUE)
}
...
ggsave("pubs_depth.png", 
       ...
       path = here ("Extracted_data_analysis",                        "tropical",                        "Output-Visualization"))
```
...
platform_depth_plot <- ggplot(expanded_data, aes(x = bin_label)) +
  geom_bar(position = "stack", alpha= 0.8, fill= "darkorange") +
...
technique_decade_barplot <- ggplot(subset(dir_ind_by_decade,
  ...
  geom_bar(position="dodge", 
           stat="identity", alpha= 0.8, fill = "darkorange" ) +
...
field_merged <- tropical[,c(59:length(tropical))]
...
field_merged <- field_merged[, colSums(field_merged) > 30]
...
field_merged %<>%
  rename(`Biodiversity and \n community structure (146)` = field_commdesc,
         `Distribution and \n connectivity (171)` = field_dist,
         `Disturbances (66)` = field_impass,
         `Mapping (46)` = field_map,
         `Physiology (124)` = field_physio,
         `Phylogenesis (31)` = field_phylo,
         `Trophic ecology (37)` = field_troph,
         `Molecular ecology (120)` = field_Molecular_ecology,
         `Environmental drivers and \n spatio-temporal patterns (136)` = field_environmental_drivers
         )
colSums(field_merged)
```
...
print(paste("Number of publications where quadrat is the sampling unit or one of the sampling unit: ",
            nrow(quad_design <- subset(tropical,grepl("quad", samplingUnit)
                                       )                                                                  )
            )
)
...
group_tot_collapsed %<>%
  mutate(Taxa = case_when(Taxa == 'echino' ~ "Echinodermata",
                          ...
                          Taxa == "Symbiodiniaceae" ~ "Symbiodiniaceae",
...
```{r}
table(tropical$meso_protection)
```

## VISUALLY SURVEYED SURFACE

```{r}
tropical$visuallySurveyedSurface <- as.factor(tropical$visuallySurveyedSurface)
summary(subset(tropical, is.na(tropical$taxonomic_publication))$visuallySurveyedSurface)
```

## Observation

```{r}
print(paste("Number of manipulative experimental studies:",nrow(subset(tropical, grepl("exp", tropical$observationalORexperimental)))))

print(paste("Number of manipulative experimental studies in the field of restoration:", nrow(subset(tropical, grepl("exp", tropical$observationalORexperimental) & tropical$field_rest != 0)
                                                                                            )
            )
      )
```
...
print(paste("# Publications that included temporal replicates:", nrow(subset(tropical, grepl("yes", temporalReplicates)))))
```

```{r  # Publications relying on a manipulative experiment that included temporal replicates}
print(paste("# Publications that included temporal replicates:", nrow(subset(tropical, (grepl("exp", tropical$observationalORexperimental) & grepl("yes", temporalReplicates))))))
```

## Health

```{r}
health <- subset(tropical, grepl("Health", indicators) | grepl("Disease", indicators))
...
```{r}
rov <- subset(tropical, grepl("rov", meso_samplingPlatform) )
...
tropical$taxonomic_publication <- as.factor(tropical$taxonomic_publication )
